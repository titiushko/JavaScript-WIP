...
I use some of these on my site, so feel free to add the effects to yours. Included are:
 ...
 Menu Repositioning: Stops menus from displaying offscreen.
  ...
  Show Default Menu: Shows a default menu on load and when no others are visible.
...
//  3) Optionally pass the 'minTop' parameter to stop it scrolling from the very
//     top of the page. Just play with this a little :).
//  4) Paste the script below at the end of fsmenu.js
...
{
 floatElement('abcdef', 0);
 // ADD OTHER PAGE ELEMENTS CONTAINING MENUS HERE.
};

function floatElement(containerID, minTop)
{
 var container = getRef(containerID).parentNode;
 if (!container) return;
 var scrY = (typeof window.pageYOffset == 'number' ? window.pageYOffset :
  (document.body ? document.body.scrollTop || document.documentElement.scrollTop : 0));
 container.style.paddingTop = Math.max(0, scrY - minTop) + 'px';
};
...
FSMenu.prototype.ieSelBoxFixShow = function(mN) { with (this)
{
 ...
 // Position and show it on each call, and show the menu to get its position.
 var mDisp = m.lyr.ref.style.display;
 m.lyr.ref.style.display = 'block';
 with (m.ifr.style)
 {
...
FSMenu.prototype.setLinkFading = function(linkClasses, linkSpeed)
{
 ...
 // Find the root menu.
 var mRoot = null;
 for (var m in this.menus) if (this.menus[m].isRoot) mRoot = this.menus[m];
 if (!mRoot) return;
 ...
 while (i--)
 {
  if (!links[i].id) links[i].id = this.myName + '-linkfade-' + i;
  links[i].__lf_timer = 0;
  links[i].__lf_count = 0;
  addEvent(links[i], 'mouseover', new Function(this.myName + '.linkFade("' + links[i].id + '", 1)'));
  addEvent(links[i], 'mouseout', new Function(this.myName + '.linkFade("' + links[i].id + '", 0)'));
 }
};
...
// If we're hiding, delay until the link is no longer highlighted.
 var fadeOK = doShow || !link.className || !this.cssLitClass ||
  (link.className.indexOf(this.cssLitClass) == -1);

 if (fadeOK)
 {
  // Find a class for this link, or default to "standard". TODO: Regex.
  var linkClass;
  for (var c in this.linkClasses)
  {
   if (link.className.indexOf(c) > -1)
   {
    linkClass = c;
    break;
   }
  }
  if (!linkClass) linkClass = this.linkClasses.standard;
  if (!linkClass) return;
  var dim = linkClass.dim, lit = linkClass.lit;

  // Increment the fading counter in the proper direction and speed.
  link.__lf_count = Math.max(0, Math.min(link.__lf_count+(2*doShow-1)*this.linkSpeed, 100));
  ...
  for (var i=0; i<3; i++)
  {
   ...
   col += parseInt(dim[i] + (lit[i]-dim[i])*(link.__lf_count/100)).toString(16);
  }
...
FSMenu.prototype.activePageHighlight = function(elm)
{
 ...
 for (var i = 0; i < links.length; i++) {
  ...
  if ((links.item(i).href.indexOf(location.pathname) > -1) &&
      (!chosen || links[i].href.length > chosen.href.length)) {
    chosen = links[i];
  }
 }

 while (chosen && chosen.className != 'menulist') {
  // Cool hack: uncomment to work with showDefaultMenu() below :).
  // if (chosen.nodeName.toLowerCase() == 'ul') this.defaultMenuID = chosen.id;
...
addEvent(window, 'load', new Function('listMenu.activePageHighlight("listMenuRoot")'));
...
// SHOW DEFAULT MENU: Will show a given first-level menu by default when the
// page loads and whenever no other menus are currently visible.
// To activate:
//  1) Paste after your menu data.
//  2) Edit the "Activation" as instructed below.

FSMenu.prototype.showDefaultMenu = function(mN) {
	if (mN == this.defaultMenuID) {
		var obj = this;
		setTimeout(function() {
			obj.onhide();
		}, 10);
	...
	for (var m in this.menus) {
		if (this.menus[m].visible && !this.menus[m].isRoot) {
			allHidden = false;
		}
	}
	if (allHidden) {
		this.show(this.defaultMenuID, this);
		this.menus.listMenuRoot.over();
	}
};

/* Activation: Set defaultMenuID here to the ID of a menu <ul id="..."> */
listMenu.defaultMenuID = 'id-of-UL-goes-here';
addEvent(listMenu, 'hide', listMenu.showDefaultMenu, 1);
addEvent(window, 'load', new Function('setTimeout("listMenu.showDefaultMenu()", 200)'));
